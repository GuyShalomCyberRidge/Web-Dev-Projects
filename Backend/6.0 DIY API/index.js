...
app.get("/jokes/:id", (req, res) => {
  ...
  if(jokes[id-1]){
    ...
    res.json({ error: "No jokes with this ID :("});
  }
});
...
app.get("/filter", (req, res) => {
  ...
  const jokesFound = jokes.filter((joke) => joke.jokeType == type);
  res.status(200);
  res.json(jokesFound);
})
...
app.post("/jokes", (req, res) => {
  ...
  jokes.push(newJoke);
  res.status(201);
  res.json({ response: "Successfully added joke!"});
})
...
app.put("/jokes/:id", (req, res) => {
  ...
  res.json({ response: "Successfully swapped joke!"});
})
...
app.patch("/jokes/:id", (req, res) => {
  ...
  if(req.body.text && req.body.type){
    res.json({ response: "choose either text or type to change."});
  } 
  else if(req.body.text){
    jokes[jokeID-1].jokeText = req.body.text;
    res.json({ response: "Successfully edited joke!"});
  } 
    ...
    res.json({ response: "No text or type specified in the body."});
  }
})
